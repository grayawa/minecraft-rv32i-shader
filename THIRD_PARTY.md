...
`mtd_linux.png` contains the 56,466,528-byte ROMFS generated by the rvc Buildroot configuration. Buildroot package recipes record the source version and license metadata for each selected userspace component. The fixed rvc revision contains the configuration and guest overlay used to reproduce the filesystem.
